...
export function Header({ transparent }: { transparent?: boolean }) {
  ...
  const NavItem = ({ href, icon: Icon, label, active, targetType }: any) => {
    let finalHref = href;

    // Smart Navigation Logic:
    // 1. Se estiver na homepage (/) → usar query params
    if (pathname === "/" && targetType === "transfer") {
      finalHref = "/?type=transfer";
    } else if (pathname === "/" && targetType === "rental") {
      finalHref = "/";
    }
    // 2. Se estiver em /search → navegar para /search/rental ou /search/transfer
    else if (pathname.startsWith("/search") && targetType) {
      finalHref =
        targetType === "transfer" ? "/search/transfer" : "/search/rental";
    }
    ...
    return (
      <button
        type="button"
        onClick={async () => {
          setNavLoading(finalHref);
          // If we're leaving the search page, reset search context to defaults
          if (
            pathname.startsWith("/search") &&
            !finalHref.startsWith("/search")
          ) {
            setSearchData({
              pickup:
                "Aeroporto Internacional Quatro de Fevereiro, Luanda, Angola",
              dropoff: "",
              from: undefined,
              to: undefined,
              time1: "12:00",
          ...
          // give tiny delay to allow spinner to render in some cases
          await router.push(finalHref);
          setNavLoading(null);
        }}
        className={cn(
          "flex items-center gap-2 px-6 py-2.5 rounded-full transition-all hover:opacity-90",
          active
      ...
      >
        ...
        {isLoading && (
          <span className="ml-2 animate-spin h-3 w-3 border-2 border-white rounded-full" />
        )}
      </button>
    );
  };
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              {!isSearch && (
                <span className="text-[10px] uppercase font-bold text-blue-200 tracking-widest mt-0.5">
                  Unique Experience
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-4">
            {isAuthenticated ? (
              <DropdownMenu>
                ...
                >
                  <DropdownMenuLabel className="text-[10px] font-black uppercase text-gray-400 px-2 py-1">
                    Gestão de Conta
                  </DropdownMenuLabel>
